...
import 'package:mdslens/services/network_permission_service.dart';
import 'package:mdslens/widgets/network_permission_gate.dart';
...
void main() {
  ...
  test(
    'Only permission-shaped network failures request permission recovery',
    () {
      expect(
  ...
  testWidgets('A denied operation offers recovery again on the next attempt', (
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: NetworkPermissionGate(
          app: app,
          enabled: true,
          child: const Scaffold(body: Text('MDSLens')),
        ),
      ),
    );
    ...
    await tester.tap(find.byKey(const ValueKey('network-permission-cancel')));
    await tester.pumpAndSettle();
  ...
  test('System settings channel is callable', () async {
    const channel = MethodChannel('mdslens/permissions');
    final messenger =
  ...
  test(
    'First-launch Apple permission requests are issued sequentially',
    () async {
      ...
      expect(calls, ['prepareNetworkAccess', 'requestLocalNetworkAccess']);
