...
/// Tracks one pointer-driven Layout Setup drag.  Flutter does not expose a
/// public API for force-cancelling a [Draggable] mid-gesture, so Escape marks
/// the active drag as cancelled: drop targets immediately stop accepting it,
/// the drag avatar disappears, and releasing the pointer leaves the draft
/// untouched.  The session is kept local to one Layout Setup dialog.
class _LayoutDragSession extends ChangeNotifier {
  _LayoutDragData? _active;
  bool _cancelled = false;

  bool get active => _active != null;

  /// A target asks this while Flutter completes the pointer gesture.  It must
  /// stay true through [onDragEnd] for a normal drop, because a target may be
  /// notified a fraction of a frame after the source reports completion.
  bool get acceptsDrop => !_cancelled;

  void start(_LayoutDragData data) {
    _active = data;
    _cancelled = false;
    notifyListeners();
  }
  ...
  bool cancel() {
    if (!active || _cancelled) return false;
    _cancelled = true;
    notifyListeners();
    return true;
  }
  ...
  void finish() {
    if (_active == null && !_cancelled) return;
    final wasCancelled = _cancelled;
    _active = null;
    notifyListeners();
    // Keep a cancellation alive until every DragTarget has seen the pointer
    // release. Otherwise an Esc followed immediately by pointer-up can still
    // be accepted by a target whose completion callback runs later that frame.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (_active == null && _cancelled == wasCancelled) {
        _cancelled = false;
        notifyListeners();
      }
    });
...
class ToolbarWidget extends StatelessWidget {
  ...
  void _showLayoutSetup(BuildContext ctx, AppState app) {
    ...
    final layoutVisualKeys = Map<Object, GlobalKey>.identity();
    GlobalKey visualKeyFor(Object item) => layoutVisualKeys.putIfAbsent(
          item,
          GlobalKey.new,
        );
    ...
    final dialogFuture = showDialog<void>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setState) {
          ...
          void deleteSelected() {
            if (selectedPanels.isEmpty) return;
            animateLayoutChange(() {
              deleteSelectedLayoutPanels(draftColumns, selectedPanels);
            });
          }

          void cancelLayout() => Navigator.pop(ctx);
          ...
          return GestureDetector(
            ...
            child: KeyboardSafeDialog(
              pageId: 'layout',
              escapeInterceptionListenable: dragSession,
              shouldInterceptEscape: () => dragSession.active,
              onEscape: () {
                final cancelled = dragSession.cancel();
                if (cancelled) app.setStatus('Layout drag cancelled');
                return cancelled;
              },
              maxWidth: 760,
              maxHeight: 860,
                                  ...
                                  Expanded(
                                    child: _LayoutSettlingTransform(
                                      ...
                                      child: VimLayoutFocus(
                                        ...
                                        visualBoundaryKey: visualKeyFor(
                                          draftColumns[displayColumn],
                                        ),
                                        child: Focus(
                                          key: ValueKey(
                                                                      ...
                                                                      children: [
                                                                        ...
                                                                        for (final cell
                                                                          ...
                                                                          _buildDraggableLayoutPanel(
                                                                            ...
                                                                            onEdit:
                                                                                () async {
                                                                              final panel = draftColumns[cell.sourceColumn][cell.sourceRow];
                                                                              final changed = await _editLayoutPanel(
                                                                                ctx,
                                                                                app,
                                                                                panel,
                                                                                cell.plotIndex + 1,
              ...
              actions: [
                Focus(
                  ...
                  skipTraversal: false,
                  child: VimActivatable(
                    onActivate: resetLayout,
                    child: OutlinedButton.icon(
                      key: const ValueKey('layout-reset'),
                      onPressed: resetLayout,
                ...
                Focus(
                  ...
                  child: VimActivatable(
                    ...
                    child: FilledButton.tonalIcon(
                      ...
                      onPressed: selectedPanels.isEmpty ? null : deleteSelected,
                      style: FilledButton.styleFrom(
                ...
                Focus(
                  ...
                  skipTraversal: false,
                  child: VimActivatable(
                    onActivate: cancelLayout,
                    child: TextButton(
                      key: const ValueKey('layout-cancel'),
                      onPressed: cancelLayout,
                      child: const Text('Cancel'),
                    ),
                  ),
                ),
  ...
  Widget _layoutColumnDropTarget({
    ...
    required _LayoutDragSession dragSession,
  }) {
    return AnimatedBuilder(
      animation: dragSession,
      builder: (context, _) => DragTarget<_LayoutDragData>(
        key: ValueKey('layout-column-drop-$insertionIndex'),
        onWillAcceptWithDetails: (_) => dragSession.acceptsDrop,
        onAcceptWithDetails: (details) {
          if (!dragSession.acceptsDrop) return;
          onLayoutChange(() {
        ...
        builder: (context, candidates, rejected) {
          ...
          final active = candidates.isNotEmpty && dragSession.acceptsDrop;
          return Tooltip(
  ...
  Widget _layoutDragHandle({
    ...
    return Draggable<_LayoutDragData>(
      ...
      feedback: feedback,
      onDragStarted: () => dragSession.start(data),
      onDragEnd: (_) => dragSession.finish(),
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) => DragTarget<_LayoutDragData>(
        ...
        builder: (context, candidates, rejected) {
          ...
          return Tooltip(
            message: columnDrop
                ? 'Insert this column here'
                : 'Insert this panel here',
            child: AnimatedContainer(
              duration: const Duration(milliseconds: 160),
              height: active ? 12 : 7,
  ...
  Widget _layoutDragFeedback(
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) => IgnorePointer(
        child: Opacity(
          opacity: dragSession.acceptsDrop ? 1 : 0,
          child: Transform.scale(
            scale: 1.03,
            alignment: Alignment.topCenter,
            child: Material(
    ...
    return Expanded(
      child: VimLayoutFocus(
        ...
        child: Focus(
          ...
          debugLabel: 'layout-panel-$sourceColumn-$sourceRow',
          onKeyEvent: (node, event) {
            if (handleVimLayoutNavigationKey(
            ...
            builder: (focusContext) => _LayoutSettlingTransform(
              ...
              child: RepaintBoundary(
                ...
                child: LongPressDraggable<_LayoutDragData>(
                  ...
                  onDragStarted: () => dragSession.start(dragData),
                  onDragEnd: (_) => dragSession.finish(),
                  childWhenDragging: Opacity(
                    opacity: 0.22,
                    child: Container(
                  ...
                  child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        border: Border.all(
                          color:
                              selected ? colors.primary : Colors.grey.shade400,
                          width: selected ? 2 : 1,
                        ),
                        borderRadius: BorderRadius.circular(6),
                      ...
                      child: _buildLayoutPanelPreview(
                        ...
                        dragHandle: _layoutDragHandle(
                          context: context,
                          key: ValueKey(
                            'layout-panel-drag-handle-$panelNumber',
                          ),
                          data: dragData,
